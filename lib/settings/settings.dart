...
class SettingsController {
  ...
  ValueNotifier<double> musicVolume = ValueNotifier(1); //0-1
  ...
  void toggleMusicOn() {
    if (audioOn.value) {
      ...
      _log.info("Ignoring music toggle because audio is muted");
    }
  }
  ...
  void toggleSoundsOn() {
    if (audioOn.value) {
      ...
      _log.info("Ignoring sound toggle because audio is muted");
    }
  }

  void cyclicVolumeIncrease() {
    if (musicOn.value && audioOn.value) {
      musicVolume.value = (musicVolume.value < 0.3)
          ? musicVolume.value = 0.4
          : (musicVolume.value < 0.6)
              ? musicVolume.value = 1.0
              : musicVolume.value = 0.15;
    }
  }
